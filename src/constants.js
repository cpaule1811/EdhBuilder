export const REQUEST_DECKLIST_PENDING = 'REQUEST_DECKLIST_PENDING'
export const REQUEST_DECKLIST_SUCCESS = 'REQUEST_DECKLIST_SUCCESS'
export const REQUEST_DECKLIST_FAILED = 'REQUEST_DECKLIST_FAILED'
export const INITIAL_DECKLIST = 'INITIAL_DECKLIST'
export const UPDATE_DECKLIST = 'UPDATE_DECKLIST'

export const REQUEST_MENU_STATUS = 'REQUEST_MENU_STATUS'
export const REQUEST_SIDEBAR_STATUS = 'REQUEST_SIDEBAR_STATUS'

export const REQUEST_USER = 'REQUEST_USER'

export const REQUEST_USER_PENDING = 'REQUEST_USER_PENDING'
export const REQUEST_USER_SUCCESS = 'REQUEST_USER_SUCCESS'
export const REQUEST_USER_FAILURE = 'REQUEST_USER_FAILURE'
export const REQUEST_USER_SIGNOUT = 'REQUEST_USER_SIGNOUT'
export const UPDATE_USER = 'UPDATE_USER'



export const profile = "data:image/png;base64,"