...
function EditorSidebar() {
  const { decklist, deckDetails, isPending } = useSelector(
  ...
  const { partner, cardImage, cardImagePartner, commander, userID } =
    deckDetails;

  const mtgMateUrl = () => {
    const cardsToAdd = decklist
      .map((item) => {
        return item.cardName;
      })
      .join("%0D%0A")
      .concat(`%0D%0A${commander}`);
      ...
      {!isPending && (
        <div className={`editor-sidebar-container`}>
          <div className="deck-type">
            {partner ? "Partners" : "Commander"} ({totalQuantity()})
          </div>
          <img
            ...
            src={imageSwitch ? cardImage : cardImagePartner}
            alt="magic the gathering card"
          />
          {partner && (
            <img
              className="commander-image partner-image"
              src={imageSwitch ? cardImagePartner : cardImage}
              alt="magic the gathering card"
            />
          )}
          <div className="purchase-wrapper">
            <div className="budget">Est. $Aus: {totalPrice()}</div>
