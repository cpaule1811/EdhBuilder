...
function SearchBar({ url }) {
  ...
  const active = (target) => {
    if (location === target) return "icon-container active";
  ...
  const fetchSearchedCards = (val) => {
    setSearchField(val);
    fetch(`${import.meta.env.VITE_API_URL}search?name=${val}`)
      ...
      <div className="search-form-container">
        <div className="search-form">
          <div className="icon-container left">
            <img src={search} alt="icon" width="25px" height="25px" />
          </div>
          <input
            onChange={(e) => fetchSearchedCards(e.target.value)}
          ...
          <div className="view-options right">
            <div className={active(`/decklist/${deckId}`)}>
              <div>
                <Link to={`${url}`}>
                  <img src={list} alt="icon" width="25px" height="25px" />
                </Link>
              </div>
              <div className="search-tab" hidden>
                LIST
              </div>
            </div>
            <div className={active(`/decklist/${deckId}/imageview`)}>
              <div>
                <Link to={`${url}/imageview`}>
                  <img src={image} alt="icon" width="25px" height="25px" />
                </Link>
              </div>
              <div className="search-tab" hidden>
                IMG
            ...
            <div className={active(`/decklist/${deckId}/recommended`)}>
              <div>
                <Link to={`${url}/recommended`}>
                  <img src={like} alt="icon" width="25px" height="25px" />
                </Link>
              </div>
              <div className="search-tab" hidden>
                Rec
