...
function Recommended() {
    ...
    useEffect(() => { 
        fetch(`https://edh-builder-api-m7vk6.ondigitalocean.app/recommend/${deckDetails.commander}/${deckDetails.partner}`)
    ...
    return (
            ...
            recommendedCards.map((item, i) => { 
                if (item[1].length) {
                return <div key={i}><div className="title"><span id="title">Top {item[0]}</span></div>
                <CardGallery Cards={item[1]} view="add"/> </div>
                } 
                return null
            })
