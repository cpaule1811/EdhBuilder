...
function AddCards() {
  ...
  const sendJsonFile = (e) => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}jsonentrys`, {
      method: "POST",
      body: fd,
    });
  };
  ...
  const fetchNewSet = (e) => {
    e.preventDefault();
    fetch(
      `https://api.scryfall.com/cards/search?order=set&q=e%3A${setId}&unique=prints`
    )
      .then((data) => data.json())
      .then((resp) => {
        const { data } = resp;
        const cardsToAdd = data.map((item) => {
          return {
            ...
            type: item.type_line,
            price:
              Number(item.prices.usd) < 0.5
                ? (0.5).toFixed(2)
                : Number(item.prices.usd) * (1.65).toFixed(2),
            cmc: item.cmc,
            modal: item.layout,
            imageUrl:
        ...
        fetch(
          `${import.meta.env.VITE_API_URL}updateentries`,
          {
            method: "POST",
            headers: {
    ...
    <div className="background">
      <div className="username-container">
        <form>
          ...
          <input
            type="submit"
            onClick={(e) => fetchNewSet(e)}
            className="signin-button"
          />
          <p style={{ margin: "5px" }}>
            {success && "entrys successfully updated"}
          </p>
        </form>
        <form>
          <h1>Update Entrys</h1>
            ...
            <div className={`drop-area ${isDragActive && "drag-active"}`}>
              ...
              {jsonFile ? (
                <div className="uploaded-file">
                  ...
                  <button onClick={() => setJsonFile("")} className="clear">
                    x
                  </button>
                </div>
              ) : (
