...
export const requestDecklist = (state = initialStateActiveDecklist, action) => {
  switch (action.type) {
    ...
    case REQUEST_DECKLIST_SUCCESS:
      return {
        ...state,
        deckDetails: action.payload[0],
        decklist: action.payload[1],
        sideboard: action.payload[2],
        authorised: action.payload[3],
        isPending: false,
      };
    ...
    case UPDATE_DECKLIST:
      return {
        ...state,
        decklist: action.payload[0],
        sideboard: action.payload[1],
      };
