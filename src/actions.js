...
export const requestDecklist = (deckId, userId) => (dispatch) => {
    ...
    fetch(`https://edh-builder-api-m7vk6.ondigitalocean.app/requestdeck/${deckId}/${userId}`, { 
        method: 'GET',
        headers: {
      ...
      .catch(error => dispatch({ type: REQUEST_DECKLIST_FAILED, payload: error }))
}
...
export const updateDecklist = (appendedDecklist, appendedSideboard) => { 
    return { type: UPDATE_DECKLIST, payload: [appendedDecklist, appendedSideboard] }
}
...
export const requestUserGoogle = (res) => (dispatch) => { 
    ...
    if (res){
        ...
        .catch(error => { dispatch({ type: REQUEST_USER_FAILURE, payload: error }) })
      } 
}
...
const fetchProfile = (dispatch, token, data) => { 
    if (data && data.userID) {
        fetch(`https://edh-builder-api-m7vk6.ondigitalocean.app/profile/${data.userID}`, { 
            method: 'GET',
            headers: {
                'content-Type': 'application/json',
                'Authorization': token
...
export const requestUser = (signinValues) => (dispatch) => { 
    ...
    .then(user => { 
        if (user.success && user.userID) { 
            ...
            dispatch({ type: REQUEST_USER_FAILURE, payload: { error: user, errorRegister: true }})
        }
